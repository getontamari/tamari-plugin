...
2. Set the working agreement, in your own words: they build the app however they like; you will wire it for Tamari as you go — SQLite → the injected `DATABASE_URL`, the startup contract (bind `PORT` on `0.0.0.0` **before** connecting to anything; a `/healthz` route that answers 200 while the database is still connecting; one JSON object per log line with a `severity` field), the trusted `x-tamari-user-id` / `x-tamari-role` / `x-tamari-user-email` identity headers, and a `tamari.json` manifest — so `/tamari:deploy` just works, and the code keeps running locally exactly as before.

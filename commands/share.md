...
- `revoke <email>` → confirm removal (if `removed: false`, the record was already gone — say so). Note that a session already open is cut off within ~10 seconds, and the change is recorded in the audit trail.
